...
description: Subscribe the state changes of a store
icon: Glasses
---
...
```tsx
useWatch(
  () => [counterStore, userStore] as const, // ✅ use `as const` to infer the type
  ([counter, user]) => {
    console.log("counter and user changed", counter.value, user.value);
  }
);
```
...
`useWatchEffect` is similar to `useWatch`, but the effect runs immediately when build the component.
...
And also support cleanup function.
