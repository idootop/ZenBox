...
| **useStore + pick** | `useStore(store, { pick: ["name", "email"] })` | ✅ Same performance, more verbose |
| **useStore**        | `useStore(store)`                              | ❌ Re-renders on any change       |
...
```tsx
// ❌ Bad: Pick nothing (same as useStore)
...
- [`useStore`](./useStore) - Full store subscription
